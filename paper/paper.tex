...
A supplementary corpus, collected after the analysis above was fixed, supports one check reported in Sec.~\ref{sec:diag-results}. It holds 17 records, 49 minutes and 29{,}524 samples at 10~Hz, reaches 6.0~m/s and includes reverse throttle, so it covers the upper half of the configured operating range that the identification trial never enters. Much of its rudder activity occurs in the autopilot's rate-controlled mode, where the logged rudder is a controller output and the firmware applies its own speed scaling. Those records are excluded from what follows, since identifying a speed-dependent rudder gain from them would partly recover the controller rather than the vehicle.
