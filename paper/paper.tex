...
The result we claim is the gap between one-step and recursive prediction, not the ranking of the three levels. That gap is measured within a single model on identical data, so it is not affected by how the models were fitted. One-step prediction reads the measured state at every sample, so a model can score 0.998 while having learned little more than that the state changes slowly. Recursive simulation removes that support and the per-step increment errors accumulate. The same gap appears on Level~3's own held-out validation set, where speed $R^2$ falls from 0.998 to 0.836, so it is a property of the model rather than of one trial.

The apparent ranking in Fig.~\ref{fig:combined} will not carry that weight. Levels~1 and~2 are scored on the trial they were identified from, while Level~3 is scored on a trial it was not fitted to, so part of the separation is the ordinary in-sample advantage rather than an effect of model structure. We therefore make no quantitative claim that structure outperforms the black box. What the figure does establish is that the structured models stay bounded while the black box does not, which follows from the drag term rather than from the fit.
