...
Preliminary results from field experiments demonstrate the approach. Levels~1 and~2 are identified from a single step-response trial; Level~3 is trained on a larger corpus of manual-mode records and evaluated on that same trial. First-order SISO models achieve $R^2=0.93$ (surge, $\tau=1.95$ s) and $R^2=0.83$ (yaw rate, $\tau=0.47$ s), providing direct controller tuning parameters. Cross-model diagnostics identify quadratic drag as the dominant surge nonlinearity and reveal that speed-dependent rudder effectiveness, a key coupling for underactuated control design, is not resolvable from a single short trial, motivating targeted data collection. A margin-based diagnostic makes the sufficiency question computable, reporting the speed range over which the first-order model is sufficient for the control design. The NARX model is where data curation proves decisive. Trained on the corpus as supplied it is unusable in simulation, settling below zero at a throttle where the vessel measurably does 1.22~m/s. One source log, 27\% of the training rows, is a stationary vessel; removing it and two others that show no response to thrust raises recursive simulation on the comparison trial from $R^2=-1.99$ to $+0.47$ in speed and $+0.26$ to $+0.85$ in yaw rate, and brings the settling speed to 1.30~m/s. What remains is an envelope limit rather than a defect: the curated network plateaus near 1.3~m/s because the corpus holds almost no sustained fast running, while the structured levels extrapolate correctly because their equilibrium follows from the equation. High-speed segments are retained for subsequent regime-spanning analysis.
\end{abstract}
...
\end{align}
with $u$ the normalized throttle and $\delta$ the normalized rudder. The yaw channel keeps a constant $k_\delta$, which is a local approximation rather than a physical claim: rudder force grows with the square of flow speed, so $k_\delta$ must vary with forward speed, and the value identified here is the one holding near the speed at which the rudder steps were flown. Diagnostic~1 returns to this.
...
Level~3 is a compact black-box reference point in the comparison and no claim is made that this architecture is optimal. Its role is to bound the benefit of added complexity at the available data scale.

\subsection{Evaluation Protocol}

Every fit reported here is by simulation. The model is initialized once from measured history and thereafter runs on its own output, replaying the throttle and rudder actually recorded, to the end of the segment without further correction. Level~3 initializes from 2~s of measured history, the span of its longest input tap; the structured levels need one sample. No controller is present: the commands are those the operator issued during the trial, replayed open loop, so the question asked is whether the model can stand in for the vessel.

Fit is reported per channel as $R^2 = 1 - \sum_k (y_k - \hat y_k)^2 / \sum_k (y_k - \bar y)^2$ against the measured signal, so zero is the score of predicting the channel mean and negative values are worse than that. Levels~1 and~2 are scored in-sample on the trial they were identified from, and the Level-3 figures state which partition each uses, since the two are not comparable on equal terms.
...
\emph{Diagnostic 1, speed-dependent rudder gain.} Rudder force grows with the square of flow speed, so steering authority varies with forward speed, and Sec.~\ref{sec:intro} treats that coupling as what makes rudder-steered craft the interesting design case. The diagnostic asks whether the available data resolves the effect well enough to carry it into the model. Replace the constant $k_\delta$ in \eqref{eq:l2yaw} with a speed-scaled $k_\delta v$ and test whether the yaw-rate fit improves. Gain scheduling is well within the capability of a production autopilot, so the question is not whether the controller can accommodate the effect but whether the model can say what schedule to use; a constant $k_\delta$ cannot, which leaves the schedule to be found by tuning at each speed on the water. The alternative tested is a partial one, since Nomoto scaling gives $k_\delta \propto v^2$ with $d_r \propto v$~\cite{fossen2011handbook} while the regression scales only the input gain and only linearly, so a null here is weaker evidence than a null from the fully scaled model.
...
Trained on the corpus as supplied, Level~3 is not usable in simulation: on the common trial it returns $R^2=-1.99$ in speed and $+0.26$ in yaw rate, and Fig.~\ref{fig:combined} shows the surge trajectory sitting below zero for tens of seconds.

The cause is a property of the training corpus rather than of the model class, and it is visible in steady state. A long simulation is governed by where the model settles: hold throttle constant and whatever speed the model converges to is what the run reports. Levels~1 and~2 have an equilibrium by construction, $v=K_v u$ and $v=\sqrt{k_T u/d_2}$, and both track the measured steady speeds, with root-mean-square errors against measurement of 0.33 and 0.18~m/s. The network trained on the raw corpus settles \emph{below zero} at a throttle of 0.20, where the vessel measurably does 1.22~m/s, and its steady-state map is not monotone in throttle, which no physical vessel can be.

Inspecting the corpus per source log explains it. One record supplies 27\% of the training rows and is 12.8 minutes of a stationary vessel: 98.7\% of its samples lie below 0.1~m/s and only 2.1\% carry any throttle. Two further logs show no speed response under sustained throttle. Roughly a third of the training data therefore asserts that throttle produces no motion, and the network learned exactly that. Dropping a source log when the vessel never got under way, or showed no response to thrust, removes 46.7\% of the training rows and changes nothing else about the procedure. Simulation on the common trial then rises to $R^2=+0.47$ in speed and $+0.85$ in yaw rate, and the settling speed at a throttle of 0.20 moves from $-0.07$ to 1.30~m/s against 1.22 measured.

What remains after curation is an envelope limit rather than a defect, and Fig.~\ref{fig:steady} shows its shape. The curated network plateaus near 1.3~m/s for any throttle above 0.15, returning 1.24~m/s at a throttle of 0.39 where the vessel does 2.09, because the slow-region corpus is capped at 3.14~m/s and holds almost no sustained fast running. A network can only locate the speed at which acceleration vanishes if the data visited it. The structured levels do not share the limit: their equilibrium follows from the form of the equation, so it is defined at operating points no trial ever reached.

This is the sense in which the comparison in Fig.~\ref{fig:combined} should be read. Within the envelope the corpus covers, the curated network is a serviceable simulator. Outside it, the structured models are the only ones that extrapolate, and that, rather than any deficiency of neural models, is what earns them their place here.
...
Taken together, the Level-1 surge model supports the control design only over roughly $[0.35, 1.28]$~m/s, under half of the tested envelope. The two bounds are not equally firm. The lower one follows from $\|\ell T\|_\infty = 1$ and has the closed form above, so it is a property of the two models. The upper one depends on choosing 50\% of design bandwidth as the point where degradation becomes unacceptable, which is a convention rather than a result: a 70\% criterion moves it to 1.03~m/s and a 30\% criterion to 1.91~m/s. What the diagnostic supplies is the curve in Fig.~\ref{fig:diagnostic4}(b) and the observation that degradation is well under way inside the tested envelope; the single number requires the designer to name a tolerance. Both bounds are insensitive to the design knob, since sweeping $\lambda$ from 0.5 to 3~s moves $v_{\text{hi}}$ only between 1.23 and 1.61~m/s and leaves $v_{\text{lo}}$ unchanged.
...
\begin{figure*}[t]
  ...
  \\caption{All three levels simulated on the common 130~s trial, with Level~3 trained on the curated logs. Level~3 begins after 2~s of measured lag initialization, so it scores 1{,}279 rows against 1{,}300 for Levels~1 and~2, and all three are scored on those same rows.}
  \label{fig:combined}
\end{figure*}
...
No one of the three models exposes all of this. Level~1 alone gives no reason to suspect the drag is quadratic. Level~2 alone does not reveal that its extra structure is inert for yaw. Level~3 alone would have carried the stationary records into the model unremarked.
...
The Level-3 result is the paper's sharpest practical lesson, and it is about data rather than architecture. A third of the supplied corpus recorded a vessel that was not moving, and a network fitted to it concluded that throttle does nothing. No amount of care in the training recipe recovered from that; removing the offending logs did, immediately. Curation on a physical criterion, that the vessel demonstrably responds to its controls, is not a preliminary to system identification here but part of it.
...
The defensible reading is about excitation rather than model class. What the network lacks is an equilibrium, and it lacks one because the records it learned from hold throttle steady for less than a time constant. A structured model does not need the experiment to contain steady running, because setting $\dot v=0$ in \eqref{eq:l2surge} returns $v=\sqrt{k_T u/d_2}$ whether or not any trial ever sat there. That is the concrete advantage of writing the model as an equation of motion, and it is a statement about how the learning problem was posed rather than about neural networks.

The same theme runs through Diagnostic~1, where the trial could not separate $\delta$ from $\delta v$, and through Diagnostic~4, where sufficiency is a property of the operating point. In each case what the model can tell a designer is bounded by what the experiment made visible.
