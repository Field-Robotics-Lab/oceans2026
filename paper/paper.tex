...
It is tempting to read this as showing that physical structure beats data volume, since Level~3 saw 28{,}023 training samples against the 1{,}300 used for Levels~1 and~2 and still simulated worst. We do not make that claim. The three levels were fitted and scored under different protocols, so the comparison cannot separate the contribution of structure from the contribution of sampling, and the sensitivity of each level to training-set size is unmeasured here. The defensible statement is narrower and does not depend on the comparison: a model can be the best one-step predictor available and still be unusable as a simulator, so the two properties must be reported separately.
