...
\end{equation}
mapping normalized throttle to forward speed and normalized rudder to yaw rate. Both are identified as discrete ARX(1,1)% CLAUDE: Define the ARX(1,1) acronym and notation on first use.
 models by  least squares on the 10~Hz record and converted to continuous time. The parameters are immediately usable: $\tau$ sets the achievable loop bandwidth % CLAUDE: closed-loop bandwidth?
  and $K$ sets the loop gain, so a PI controller follows directly. % CLAUDE: Do you mean this implies a PI compensator?
...
The value of the spectrum models is the comparison across levels rather than the three fits individually. Scoring one structure on held-out data is a weaker check here, % CLAUDE: Revise "Scoring one structure on held-out data is a weaker check here, " for clarity
because a maneuvering model can pass a prediction test while its coefficients remain physically meaningless~\cite{alexandersson2022system}, and a coefficient that cannot be interpreted cannot inform a control gain. Four diagnostics compare across structures instead. The first three ask whether a term is present in the data. The fourth asks whether it changes the loop design.
