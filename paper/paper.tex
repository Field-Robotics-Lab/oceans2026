...
On the common trial Level~3 reaches one-step $R^2$ of 0.998 in speed and 0.989 in yaw rate. Under recursive simulation on that same trial it falls to $-1.988$ in speed and $0.259$ in yaw rate, a negative $R^2$ meaning the model predicts worse than the mean of the measured signal. Fig.~\ref{fig:combined} shows the surge trajectory drifting below zero and staying there for tens of seconds.
...
This bounds what Fig.~\ref{fig:combined} can be said to show. It is not a comparison of structured against black-box modeling, because it places two models that possess an equilibrium against one that does not. We make no claim from it about model class. What it does show is narrower and, for a practitioner, more useful: a model can be the best one-step predictor available and still be unusable in the closed loop, and reporting one-step accuracy alone will not reveal that.
...
The defensible reading is about excitation rather than model class. What the network lacks is an equilibrium, and it lacks one because the records it learned from hold throttle steady for less than a time constant. A structured model does not need the experiment to contain steady state, because its equilibrium follows from the form of the equations: setting $\dot v=0$ in \eqref{eq:l2surge} gives $v=\sqrt{k_T u/d_2}$ whether or not any trial ever sat there. That is the concrete sense in which structure earns its place at this data scale, and it is a claim about what the experiment excited rather than about neural networks.
